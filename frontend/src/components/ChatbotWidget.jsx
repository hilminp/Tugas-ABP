    ...
    const ADVISORY_TEXT = 'Chatbot ini dibuat untuk menemani, bukan menggantikan bantuan profesional. Jika terasa berat, hubungi orang tepercaya atau tenaga ahli ya.';
    ...
    const ChatbotWidget = ({ forceOpen = false, hideToggle = false, onClose }) => {
        ...
        const [showAdvisoryPopup, setShowAdvisoryPopup] = useState(forceOpen);
        const [showAdvisoryMessage, setShowAdvisoryMessage] = useState(true);
            ...
            <div className="chatbot-widget">
                {(forceOpen || isOpen) && (
                    <section className="chatbot-panel" aria-label="Chatbot pilihan emosi">
                        ...
                        {showAdvisoryPopup && (
                            <div
                                className="chatbot-advisory-backdrop"
                                role="presentation"
                                onClick={handleCloseAdvisory}
                            >
                                <div
                                    className="chatbot-advisory-popup"
                                    role="dialog"
                                    aria-modal="true"
                                    aria-labelledby="chatbot-advisory-title"
                                    onClick={(event) => event.stopPropagation()}
                                >
                                    <button
                                        type="button"
                                        className="chatbot-advisory-close"
                                        onClick={handleCloseAdvisory}
                                        aria-label="Tutup himbauan"
                                    >
                                        ✕
                                    </button>
                                    <span className="chatbot-advisory-icon" aria-hidden="true">i</span>
                                    <h4 id="chatbot-advisory-title">Sebelum mulai curhat</h4>
                                    <p>{ADVISORY_TEXT}</p>
                                    <button
                                        type="button"
                                        className="chatbot-advisory-action"
                                        onClick={handleCloseAdvisory}
                                    >
                                        Mengerti
                                    </button>
                            ...
                            <div className="chatbot-messages" aria-live="polite" aria-label="Riwayat percakapan chatbot">
                                {showAdvisoryMessage && (
                                    <div className="chatbot-message-row bot">
                                        <div className="chatbot-bubble bot chatbot-bubble-advisory">
                                            <button
                                                type="button"
                                                className="chatbot-bubble-advisory-close"
                                                onClick={() => setShowAdvisoryMessage(false)}
                                                aria-label="Tutup catatan penting"
                                            >
                                                ✕
                                            </button>
                                            <span className="chatbot-bubble-advisory-label">Catatan penting</span>
                                            {ADVISORY_TEXT}
                                        </div>
                                    </div>
                                )}
                                ...
                                {!showAdvisoryMessage && (
                                    <div className="chatbot-advisory-restore-row">
                                        <button
                                            type="button"
                                            className="chatbot-advisory-restore"
                                            onClick={() => setShowAdvisoryMessage(true)}
                                        >
